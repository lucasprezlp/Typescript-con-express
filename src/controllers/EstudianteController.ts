...
export const consultarApellido = async (req: Request, res: Response): Promise<Estudiante | null> => {
    const { apellido } = req.body;
    try {
        const estudianteRepository = AppDataSource.getRepository(Estudiante);
        const estudiante = await estudianteRepository.findOne({ where: { apellido } });
